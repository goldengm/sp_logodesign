...
import {
  ResizeCallbackData,
  useResizeLayer,
} from "../layers/hooks/useResizeLayer";
import { isImageLayer, isVideoLayer } from "../ultils/layer/layers";
import {
  RotateCallbackData,
  useRotateLayer,
} from "../layers/hooks/useRotateLayer";
...
export interface PageProps {
  ...
  onSavedImage: (thumbnail: Blob | null) => void;
}
const DesignPage: ForwardRefRenderFunction<HTMLDivElement, PageProps> = (
  { pageIndex, width, height, transform, onSavedImage },
  ref
) => {
  ...
  useEffect(() => {
    const saveImage = async () => {
      if (displayRef.current && pageIndex == 0) {
        let ratio = 400 / width;
        if (ratio > 1) ratio = 1;
        try {
          const thumbnailData = await toBlob(displayRef.current, {
            pixelRatio: ratio,
          });
          onSavedImage(thumbnailData);
        } catch (e) {
          window.alert("Cannot Save thumbnail: " + (e as Error).message);
        }
      }
    };
    if (savedDate != null) saveImage();
  }, [savedDate]);
