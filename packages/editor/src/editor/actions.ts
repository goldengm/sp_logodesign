  ...
  return {
    ...
    addSvgLayer(
      ...
      const defaultStrokeColor =
        element.getAttribute("stroke") || element.style.stroke || "none";
      const defaultFillColor =
        element.getAttribute("fill") || element.style.fill || "#000000";
      if (list.length > 1) {
        for (let i = 0; i < list.length; i++) {
          ...
          if (id) {
            ...
            if (verifyId && verifyId.length >= 2) {
              ...
              if (paths.length > 0) {
                const stroke =
                  paths[0].getAttribute("stroke") || defaultStrokeColor;
