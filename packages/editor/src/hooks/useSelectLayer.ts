...
export const useSelectLayer = ({
  ...
  const [tmpSelected, setTmpSelected] = useState<
    | { selectedPage: number; selectedLayers: Record<LayerId, BoxData> }
    | undefined
  >();
  ...
  const pageRect = () => {
    return pages.map(({ layers }, pageIndex) => ({
      layers: [...layers.ROOT.data.child, "ROOT"]
        .reverse()
        .reduce((acc, layerId) => {
          const {
            id,
            data: { props, waterMark },
          } = layers[layerId];
          if (waterMark == true) return acc; //waterMark - seth
          ...
          const matrix = new WebKitCSSMatrix(
            getTransformStyle({ rotate: props.rotate })
          );
  ...
  const processSelect = useCallback(
    (
      ...
      if (selectionBoxRef.current && frameRef.current) {
        ...
        const left =
          clientX + scrollLeft > selectionRef.clientX
            ? selectionRef.clientX
            : clientX + scrollLeft;
        const top =
          clientY + scrollTop > selectionRef.clientY
            ? selectionRef.clientY
            : clientY + scrollTop;
        ...
        pageRect().forEach(({ layers }, pageIndex) => {
          ...
          layers.forEach(({ id: layerId, delta: layerRect }) => {
            if (
              ...
              selectedLayers[layerId] = {
                boxSize: {
                  width:
                    pages[selectedPage].layers[layerId].data.props.boxSize
                      .width * scale,
                  height:
                    pages[selectedPage].layers[layerId].data.props.boxSize
                      .height * scale,
                ...
                position: {
                  x:
                    ...
                    frameRect.x +
                    (frameRef.current?.scrollLeft || 0) +
                    pages[selectedPage].layers[layerId].data.props.position.x *
                      scale,
                  y:
                    rect.y -
                    frameRect.y +
                    (frameRef.current?.scrollTop || 0) +
                    pages[selectedPage].layers[layerId].data.props.position.y *
                      scale,
                },
          ...
          if (
            ...
            selectedLayers["ROOT"] = {
              boxSize: {
                width:
                  pages[selectedPage].layers.ROOT.data.props.boxSize.width *
                  scale,
                height:
                  pages[selectedPage].layers.ROOT.data.props.boxSize.height *
                  scale,
              },
              position: {
                x:
                  rect.x -
                  frameRect.x +
                  pages[selectedPage].layers.ROOT.data.props.position.x * scale,
                y:
                  rect.y -
                  frameRect.y +
                  (frameRef.current?.scrollTop || 0) +
  ...
  const handleSelectEnd = useCallback(
    (e: MouseEvent | TouchEvent) => {
      ...
      const res = processSelect(
        { clientX, clientY },
        clientX + (frameRef.current as HTMLDivElement).scrollLeft ===
          getSelectionRef().clientX &&
          clientY + (frameRef.current as HTMLDivElement).scrollTop ===
            getSelectionRef().clientY
      );
      if (res) {
  ...
  const handleSelectStart = (e: MouseEvent) => {
    ...
    actions.setSelectData(true);
    selectionRef.current.clientX =
      clientX + (frameRef.current as HTMLDivElement).scrollLeft;
    selectionRef.current.clientY =
      clientY + (frameRef.current as HTMLDivElement).scrollTop;
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (isSelecting) {
        const res = processSelect({
          clientX:
            selectionRef.current.moveClientX || selectionRef.current.clientX,
          clientY:
            selectionRef.current.moveClientY || selectionRef.current.clientY,
        });
        if (res) {
          setTmpSelected(res);
        }
